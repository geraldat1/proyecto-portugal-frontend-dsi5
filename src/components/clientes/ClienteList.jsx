...
const ClienteList = ({ clientes, seleccionar, eliminar }) => {
  ...
  const clientesFiltrados = clientes.filter((cliente) => {
    ...
    const coincideEstado = filtroEstado === "todos" || 
      (filtroEstado === "activos" && cliente.estado === 1) ||
      (filtroEstado === "inactivos" && cliente.estado === 0);
  ...
  const totalPages = Math.ceil(clientesOrdenados.length / clientsPerPage);

  const paginate = (pageNumber) => setCurrentPage(pageNumber);

  // Estadísticas
  const clientesActivos = clientes.filter(cliente => cliente.estado === 1).length;
  const clientesInactivos = clientes.filter(cliente => cliente.estado === 0).length;
  ...
  const confirmarEliminacion = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          title: "¡Deshabilitado!",
          text: "El cliente ha sido deshabilitado.",
          icon: "success",
    ...
    <div className="p-4 bg-white rounded-3 shadow-sm">
          ...
          <Card.Body className="py-3">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <h2 className="mb-0 fw-bold display-6">{clientes.length}</h2>
              </div>
          ...
          <Card.Body className="py-3">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <h2 className="mb-0 fw-bold display-6">{clientesActivos}</h2>
              </div>
          ...
          <Card.Body className="py-3">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <h2 className="mb-0 fw-bold display-6">{clientesInactivos}</h2>
              </div>
              <div className="bg-secondary text-white rounded-circle p-3">
                <FaUserCheck size={24} />
              </div>
            </div>
            <small className="text-muted">Clientes inactivos</small>
          </Card.Body>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3 bg-light p-3 rounded shadow-sm">
        ...
        <div className="text-muted fw-medium">
          Mostrando <span className="text-primary fw-bold">{clientesOrdenados.length}</span> de 
          <span className="text-primary fw-bold"> {clientes.length}</span> clientes
        </div>
      </div>
            ...
            <tbody>
              {currentClients.length > 0 ? (
                currentClients.map((cliente) => (
                  <tr key={cliente.id} className={cliente.estado === 0 ? "text-muted bg-light" : ""}>
                    <td className="fw-bold fs-6 align-middle">{cliente.id}</td>
                    <td className="fw-medium align-middle">{cliente.dni}</td>
                    ...
                    <td className="fw-medium align-middle">{cliente.telefono}</td>
                    <td className="fw-medium align-middle">{cliente.direccion}</td>
                    <td className="fw-medium align-middle">{new Date(cliente.fecha).toLocaleDateString()}</td>
                    <td className="align-middle">
                      <Badge 
                        pill 
                        className={cliente.estado === 1 ? "bg-success fs-6" : "bg-secondary fs-6"}
                      >
                ...
                <tr>
                  <td colSpan="8" className="text-center py-5">
                    <div className="d-flex flex-column align-items-center justify-content-center">
                      ...
                      <h5 className="text-primary fw-bold">No se encontraron clientes</h5>
